...
FeedParser.prototype.handleMeta = function handleMeta (node, type, options) {
  ...
  if (normalize) {
    ['title','description','date', 'pubdate', 'pubDate','link', 'xmlurl', 'xmlUrl','author','language','favicon','copyright','generator','type'].forEach(function (property){
      meta[property] = null;
    });
    meta.cloud = {};
    meta.image = {};
  ...
  Object.keys(node).forEach(function(name){
    ...
    if (normalize) {
      ...
      case('media:category'):
        ...
        break;
      case('itunes:type'):
        var itunesType = _.get(el);
        if (['episodic', 'serial'].indexOf(itunesType) !== -1) {
          meta.type = itunesType;
        }
        break;
      } // switch end
    }
...
FeedParser.prototype.handleItem = function handleItem (node, type, options){
  ...
  if (normalize) {
    ['title','description','summary','date','pubdate','pubDate','link','guid','author','comments', 'origlink','season','episode','episodeType'].forEach(function (property){
      item[property] = null;
    });
    item.image = {};
    item.source = {};
  ...
  Object.keys(node).forEach(function(name){
    ...
    if (normalize) {
      ...
      case('pheedo:origlink'):
        ...
        break;
      case('itunes:season'):
        var season = Number(_.get(el));
        if (Number.isFinite(season) && season > 0) {
          item.season = season;
        }
        break;
      case('itunes:episode'):
        var episode = Number(_.get(el));
        if (Number.isFinite(episode) && episode > 0) {
          item.episode = episode;
        }
        break;
      case('itunes:episodetype'):
        var episodeType = _.get(el);
        if (['full', 'trailer', 'bonus'].indexOf(episodeType) !== -1) {
          item.episodeType = episodeType;
        }
        break;    
      } // switch end
    }
