...
function handleItem (node, type){
  ...
  var item = {};
  ['title','description','summary','date','pubDate','link','guid','author','comments'].forEach(function (property){
    item[property] = null;
  });
  item.source = {};
  item.categories = [];
  ...
  Object.keys(node).forEach(function(name){
    ...
    case('media:content'):
      ...
      break;
    case('enc:enclosure'): // Very rare. Skip it unless someone asks for it.
      break;      
    case('category'):
    case('dc:subject'):
    case('itunes:category'):
  ...
  if (!item.image) {
    if (node['itunes:image']) item.image = getValue(node['itunes:image'], 'href');
    else if (node['media:thumbnail']) item.image = getValue(node['media:thumbnail'], 'url');
  }
