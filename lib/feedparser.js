/**********************************************************************
 node-feedparser - Robust RSS, Atom, RDF parser for node.
...
function FeedParser () {
  ...
  self.stream.on('error', function (e){ self.handleError(e, self) });
...
FeedParser.prototype.parseFile = function(file, callback) {
  ...
  if (/^https?:/.test(file) || (typeof file == 'object' && 'protocol' in file)) {
    ...
    self._setCallback(callback);
    fs.createReadStream(file).pipe(self.stream);
  }
};
