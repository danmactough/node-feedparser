...
FeedParser.prototype.parseFile = function _parseFile (file, callback) {
  ...
  fs.readFile(file, 'utf8', function(err, data){
	if (err) {
	  ...
	  self.parseString(data, callback);
	}
  });
}

/**
 * Parse feed from a given URL
 * 
 * @param string url URL of the feed to parse
 * @param function cb A valid callback
 * @api public
  ...
  request(url, function (error, response, body) {
    if (error) {
      ...
      if (response.statusCode == 200) {
        self.parseString(body, callback);
      } else {
        self.emit('error', new Error('Unexpected status code #' + response.statusCode));
      }
    }
  });
}
